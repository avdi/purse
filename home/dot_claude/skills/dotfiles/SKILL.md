...
The rule is about not stalling on the push. It is not a license to put anything
at all into a public repo that auto-deploys to every machine. Stop and raise
these:

- **Secret values.** purse stores secret *IDs* (`~/.config/purse/secret-ids.env`)
  and resolves them at runtime via `zv`; `~/.config/shell/secrets.sh` is
...
- **Rewriting pushed history.** Rebasing your own unpushed commits onto the
  remote is the normal answer to a rejected push. Force-pushing over commits
  already on `origin` is not — other machines have them.
